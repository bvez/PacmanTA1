...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    "*** YOUR CODE HERE ***"
    self.posInicial = self.startingPosition
    self.flag=False #todavia no ha comido las 4 esquinas
...
def cornersHeuristicNonConsistent(state, problem):
  """
  ...
  x,y = state[0]
  cornersLeft = state[1]
  for cornerx,cornery in cornersLeft:
  	heuristic += abs(x - cornerx)
  	heuristic += abs(y - cornery)
