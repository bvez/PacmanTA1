...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        if (len(state[1]) ==0):
        #si esta condicion se cumple, el pacman ya comio en las 4 esquinas,
        #para regresar a su posicion inicial, se agregan las filas 308 y 309.
        #Dado que la condicion del problema es que el pacman haya comido y finaliza,
        #no se muestra como el pacman regresa a su posicion inicial, por ende,
        # se modificaron las lineas 99 y 293 de pacman.py para que el juego no termine
        	x,y = state [0]
        	if(x == self.posIniX and y==self.posIniY):
        		return True
        return False
...
def cornersHeuristic(state, problem):
    ...
    while(len(remainingCorners)>0):
    	#distance, minCorner = min([(util.manhattanDistance(node, corner), corner) for corner in unvisited])
    	minDistance = 999999
